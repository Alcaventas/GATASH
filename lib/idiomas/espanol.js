...
const smsMenuTotal2 = () => { return "TIPOS DE VS"}
...
//_autodetec.js
const smsAutodetec1 = (usuario, m) => { return `*» ${usuario}*\n*𝙃𝘼 𝘾𝘼𝙈𝘽𝙄𝘼𝘿𝙊 𝙀𝙇 𝙉𝙊𝙈𝘽𝙍𝙀 𝘿𝙀𝙇 𝙂𝙍𝙐𝙋𝙊*\n\n🔰 *𝘼𝙃𝙊𝙍𝘼 𝙀𝙇 𝙂𝙍𝙐𝙋𝙊 𝙎𝙀 𝙇𝙇𝘼𝙈𝘼:*\n*${m.messageStubParameters[0]}*`} 
const smsAutodetec2 = (usuario, groupMetadata) => { return `*» ${usuario}*\n*𝙃𝘼 𝘾𝘼𝙈𝘽𝙄𝘼𝘿𝙊 𝙇𝘼 𝙄𝙈𝘼𝙂𝙀𝙉 𝘿𝙀:*\n*${groupMetadata.subject}*`} 
const smsAutodetec3 = (usuario, m, groupMetadata) => { return `*» ${usuario}*\n*HA PERMITIDO QUE ${m.messageStubParameters[0] == 'on' ? 'SOLO ADMINS' : 'TODOS'} PUEDAN CONFIGURAR ${groupMetadata.subject}*`} 
const smsAutodetec4 = (groupMetadata, usuario) => { return `*EL ENLACE DE ${groupMetadata.subject} HA SIDO RESTABLECIDO POR:*\n*» ${usuario}*`} 
const smsAutodetec5 = (groupMetadata, m, usuario) => { return `*${groupMetadata.subject} HA SIDO ${m.messageStubParameters[0] == 'on' ? 'CERRADO 🔒' : 'ABIERTO 🔓'} POR ${usuario}*\n\n💬 *AHORA ${m.messageStubParameters[0] == 'on' ? 'SOLO ADMINS' : 'TODOS'} PUEDEN ENVIAR MENSAJES*`} 
const smsAutodetec6 = (m, groupMetadata, usuario) => { return `*@${m.messageStubParameters[0].split`@`[0]} 𝘼𝙃𝙊𝙍𝘼 𝙀𝙎 𝘼𝘿𝙈𝙄𝙉 𝙀𝙉*\n*» ${groupMetadata.subject}*\n\n✨ *𝘼𝘾𝘾𝙄𝙊𝙉 𝙃𝙀𝘾𝙃𝘼 𝙋𝙊𝙍:*\n*» ${usuario}*`} 
const smsAutodetec7 = (m, groupMetadata, usuario) => { return `*@${m.messageStubParameters[0].split`@`[0]} 𝘿𝙀𝙅𝘼 𝘿𝙀 𝙎𝙀𝙍 𝘼𝘿𝙈𝙄𝙉 𝙀𝙉*\n*» ${groupMetadata.subject}*\n\n✨ *𝘼𝘾𝘾𝙄𝙊𝙉 𝙃𝙀𝘾𝙃𝘼 𝙋𝙊𝙍:*\n*» ${usuario}*`} 
